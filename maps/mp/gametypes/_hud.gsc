...
fontPulseInit( maxFontScale )
{
	...
	if ( isDefined( maxFontScale ) )
		self.maxFontScale = min( maxFontScale, 1.03 );
	else
		self.maxFontScale = min( self.fontScale * 2, 1.03 );
	self.inFrames = 2;
	self.outFrames = 4;
}
...
fontPulse2Init( maxFontScale )
{
	self.baseFontScale = self.fontScale;
	if ( isDefined( maxFontScale ) )
		self.maxFontScale = min( maxFontScale, 0.03 );
	else
		self.maxFontScale = min( self.fontScale * 2, 0.03 );
	self.inFrames = 2;
	self.outFrames = 4;
}
...
fontPulse2(player)
{
	self notify ( "fontPulse2" );
	self endon ( "fontPulse2" );
	self endon( "death" );
