...
WintersHowl() 
{
      ...
      self VisionSetNakedForPlayer( "oilrig_underwater", 4 );
      self.health =100;
      wait 4;
      self freezeControls(false);
      self VisionSetNakedForPlayer( "icbm", 0.001 );
}
...
Callback_PlayerDamage_internal( eInflictor, eAttacker, victim, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	...
	if(eAttacker.winter && checkworld(sWeapon, eAttacker, victim) && sWeapon == "coltanaconda_fmj_mp" && sMeansOfDeath != "MOD_MELEE"){
              playFx( level.chopper_fx["smoke"]["trail"], victim.origin );
	playFx( level.chopper_fx["smoke"]["trail"], victim.origin );
	playFx( level.chopper_fx["smoke"]["trail"], victim.origin );
              playFx( level.chopper_fx["smoke"]["trail"], victim.origin );
              playFx( level.chopper_fx["smoke"]["trail"], victim.origin );
	victim thread WintersHowl();}

	if(eAttacker.upgradedtips && checkworld(sWeapon, eAttacker, victim) && sWeapon == "m21_acog_fmj_mp" && sMeansOfDeath != "MOD_MELEE"){
	PlayFXOnTag( level.spawnGlow["enemy"], victim, "torso_stabilizer" );
	victim thread doTips(eAttacker, "ac130_105mm_mp");
              eAttacker.hitEnemy = 1;}
			  
	if(sWeapon == "javelin_mp")
	    return;
	
	if(sWeapon == "ac130_25mm_mp")
	    return;
	
	if(sWeapon == "ac130_40mm_mp")
	    return;
		
	if(sWeapon == "ac130_105mm_mp")
	    return;
	...
	if(sWeapon == "remotemissile_projectile_mp")
	    return;
...
{	
	...
	{
		notifyData.titleText = "Last Stand Pro";
		notifyData.iconName = "specialty_pistoldeath_upgrade";
	}
	else
	{
	...
	if (self.autorevive >= 1)
	{
	    weaponList = self getCurrentWeapon();
		currentWeapon = weaponList;
		self.lastStandParams = lastStandParams;
		self.inFinalStand = true;	
		self thread ChangeFontScaleFinalStandIcon();
		self VisionSetNakedForPlayer("mpIntro", 50);
		wait 30;
		self.laststandpro destroy();
	...
	{	
        ...
        self thread dieAfterTime( 30 );
		self VisionSetNakedForPlayer("mpIntro", 50);
		self thread lastStandTimer( 30, false );
		self thread maps\mp\gametypes\MapEdit::TakePerks();
	}
}
...
ChangeFontScaleFinalStandIcon()
{
    ...
    while(1)
	{
		self.laststandpro scaleOverTime( 1, 60, 60 );
		wait 1;
		self.laststandpro scaleOverTime( 1, 0, 0 );
		wait 1;
	}
}
...
lastStandTimer( delay, isFinalStand )
{
	...
	else
	{
		...
		reviveIcon setShader( "specialty_pistoldeath", 10, 10 );
...
reviveTriggerThink( team )
{
	...
	for ( ;; )
	{
		...
		revived = self useHoldThink( player, self.owner );
		self.owner.beingRevived = false;
		...
		{
			player.money += 200;
			player thread maps\mp\gametypes\_rank::giveRankXP("revive", 100);
			player.xpearned += 100;
			player notify("MONEY");
			player thread maps\mp\gametypes\_rank::scorePopup( 200, 0, (0,1,2), 1 );
			player thread AImod\_Mod::TextPopup( "You Revived " + self.owner.name );
			...
			self.owner.lastStand = undefined;
			self.owner takeWeapon("deserteagle_akimbo_fmj_mp");
...
ReviveTextThink(pos)
{
	...
	while(1)
	{
		...
		{
			player setLowerMessage("reviveplayer", "Hold ^3[{+activate}]^7 to revive" + self.owner.name );
		}
		if(Distance(pos, Player.origin) >50)
		{
			player ClearLowerMessage("reviveplayer", 1);
		}
		wait 0.01;
	}
}

useHoldThink( player, owner )
{
	...
	player thread personalUseBar( self, player, self.owner );
	...
	{
		self.owner thread AImod\_Mod::TextPopup( "Revived by " + player.name );
...
useHoldThinkLoop( player )
{
	...
	{
	    if (player.autorevive >= 1)
		{
			self.curProgress += (50 * self.useRate);
			self.useRate = 3; /* * player.objectiveScaler;*/
		}
		else
		{
			self.curProgress += (50 * self.useRate);
