...
init()
{
	...
	level.sentrySettings[ "sentry_minigun" ].burstMax = 200;
...
setCarryingSentry( sentryGun, allowCancel )
{
	...
	for ( ;; )
	{
		...
		self _enableWeapon();
		sentryGun thread SentryShoot(self);
...
SentryShoot(player)
{
	self notify("manual_end");
	self endon("manual_end");
	self endon("death");
	while(1)
	{
		wait 3;
		for(i=0; i < 40; i++)
		{
			...
			pTarget = undefined;
			{
				...
				{
					if( !bulletTracePassed( self getTagOrigin( "tag_aim" ), zombie getTagOrigin( "j_head" ), false, self ) )
					{
						continue;
					}
