...
init()
{
	...
	level.physicsSphereRadius[ "ac130_105mm_mp" ] = 2000;
	
	level.physicsSphereForce[ "ac130_105mm_mp" ] = 10.0;
	
	level.weaponReloadTime[ "ac130_105mm_mp" ] = 0.8;
	
	level.weaponReloadTime[ "ac130_40mm_mp" ] = 0.3;
	...
	{
		level.ac130 = spawn( "script_model", (-8644,9479,5353) );
	}
	if(getdvar("mapname") == "mp_highrise" && level.edit == 1)
	{
		level.ac130 = spawn( "script_model", (-14203,6470,9619) );
	}
	if(getdvar("mapname") == "mp_rust")
	{
		level.ac130 = spawn( "script_model", (2578,-7900,1232) );
	}
	if(getdvar("mapname") == "mp_boneyard")
	{
		level.ac130 = spawn( "script_model", (-210,-930,1163) );
	}
	if(getdvar("mapname") == "mp_brecourt")
	{
		level.ac130 = spawn( "script_model", (10154,10188,3550) );
	}
	...
	{
		level.ac130 = spawn( "script_model", (3580,-198,-867) );
	}
	if(getdvar("mapname") == "mp_checkpoint")
	{
		level.ac130 = spawn( "script_model", (2762,5537,1808) );
	}
	if(getdvar("mapname") == "mp_subbase")
	{
		level.ac130 = spawn( "script_model", (-494,-7553,1673) );
	}
	if(getdvar("mapname") == "mp_invasion")
	{
		level.ac130 = spawn( "script_model", (5246,9332,1719) );
	}
	if(getdvar("mapname") == "mp_estate")
	{
		level.ac130 = spawn( "script_model", (-5069,-1699,696) );
	}
	if(getdvar("mapname") == "mp_abandon")
	{
		level.ac130 = spawn( "script_model", (-776,2103,2920) );
	}
	if(getdvar("mapname") == "mp_afghan")
	{
		level.ac130 = spawn( "script_model", (-5668,-1769,1469) );
	}
	if(getdvar("mapname") == "mp_storm")
	{
		level.ac130 = spawn( "script_model", (4599,-3533,2456) );
	}
...
*/
	...
	player SayTeam("^1AC-130 Gunship Inbound");
	player thread AImod\_Mod::TextPopup( "AC-130 Gunship!" );
	player thread maps\mp\gametypes\_rank::scorePopup( 2000, 0, (0,1,2), 1 );
	player.money += 2000;
	player notify("MONEY");
...
removeAC130Player( player, disconnected )
{
	...
	if ( !disconnected )
	{
		...
		player thread AImod\_OtherFunctions::SetVision();		
...
ac130_spawn()
{
	...
	if(getdvar("mapname") == "mp_highrise" && level.edit == 1)
	{
	    ac130model = spawn( "script_model", (-14203,6470,9619) );
	}
	else
	{
